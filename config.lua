...
Config.Plants = {

    {
        ...
        hash = joaat("rdr_bush_dry_thin_ba_sim"), -- Plant name as a hash value
    ...
    {
        hash = joaat("s_inv_blackberry01x"),
        name = "Blackberry",
        reward = { "blackberry" },
        minReward = 1,
        maxReward = 5
...
Config.Locations = {
    ...
    {
        name = "Blueberry",                        -- Area name
        plantModel = "rdr_bush_dry_thin_ba_sim",   -- Plant model to spawn (optional!) if setting plant plantModel and the plantModel is one set up in config.plants do not add reward here set up reward in config.plants
        coords = vector3(2046.34, -829.13, 42.96), -- Coordinates for dirt mounds/reward item objects
        timeout = 1,                               -- Custom timeout per node (optional, in minutes!
        minReward = 1,                             --(optional!)
        maxReward = 5                              --(optional!)

    },
    -- Partially customized node (uses coords to create prompt, preferrably near existing world objects) if near
...
Config.Language = {
    ...
    ItemsReceived = "You received: ",
    NoEntryFound = "No entry found in usedPoints for the given coords"
}
